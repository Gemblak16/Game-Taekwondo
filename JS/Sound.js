...
function playClickSound() {
    try {
        ...
        const targetVol = 0.9 * soundVolume; 
...
function playVictorySound() {
    try {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            filter.frequency.setValueAtTime(1500, now + timeDelay);

            const noiseGain = audioCtx.createGain();
            noiseGain.gain.setValueAtTime(0.4, now + timeDelay);
            noiseGain.gain.exponentialRampToValueAtTime(0.001, now + timeDelay + 0.4); 

            noiseNode.connect(filter);
            filter.connect(noiseGain);
            noiseGain.connect(audioCtx.destination);
            noiseNode.start(now + timeDelay);
        }
...
function playCountdownSound(isGo) {
    try {
        ...
        const targetVol = (isGo ? 0.4 : 0.3) * soundVolume; 
        gainNode.gain.setValueAtTime(targetVol, now);
        gainNode.gain.exponentialRampToValueAtTime(0.001, now + (isGo ? 0.25 : 0.12)); 
...
function playBossTokenSound() {
    try {
        ...
        const filter = document.createElement ? null : audioCtx.createBiquadFilter(); 
        ...
        const baseVolume = 0.5 * soundVolume;
        gainNode1.gain.setValueAtTime(baseVolume, now);
        gainNode1.gain.exponentialRampToValueAtTime(0.001, now + 0.5);

        gainNode2.gain.setValueAtTime(baseVolume * 0.8, now);
